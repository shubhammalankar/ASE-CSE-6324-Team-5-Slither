...
public class AntlrToCondition extends ExprBaseVisitor<Contract_Parts>{
public String DOS_Warning;
	
	public AntlrToCondition(String DOS_Warning) {
		this.DOS_Warning = DOS_Warning;
	}
	@Override
	public Contract_Parts visitIfStatement(IfStatementContext ctx) {
		  ...
		  //Find opening bracket character to check function call
		    ...
		    if (parenIndex != -1) {
		        System.out.println("Security issue at line " + line + ": Possibility of a DOS"
		        		+ " attack due to an external call inside 'if' condition. "
		        		+ "Please consider setting gas limit.");
		    }
		    i = dotIndex + 1;
		}
